...
library(DT)
library(rmarkdown)
#library(rCharts)   # does not load into R 3.4.1
#library(rsconnect)
library(dygraphs)
library(xts)
...
# read pre-loaded files

MonthlyLFSx1 <- as.data.frame(readRDS("../data/MonthlyLFSx1.rds"))
MonthlyLFSx2 <- as.data.frame(readRDS("../data/MonthlyLFSx2.rds"))
wide_UnRate <- as.data.frame(readRDS("../data/wideUnRate.rds"))
...
server <- function(input, output) {
  ...
  output$URPlot <- renderPlot({
    ...
    MonthlyLFSx1.month <- MonthlyLFSx1 %>%
      filter(Date==input$RefMonth)
    
    n1 <- ggplot(MonthlyLFSx1.month, aes(x='i', y = 'UnRate') +
          ...
          sidebarLayout(      
            ...
            # The main panel
              mainPanel(
                 ...
                 plotOutput("URPlot") # The bar graph for all Regions
                 #plotOutput("URPlot", lib="nvd3") # The bar graph for all Regions
                        )         
                    )
              ), 
